...
| C: security and AI features | **In progress.** C1 (attack library) and C2 (mutator) are done, 2026-09-26; C3 (MCP) and C4 (failure clustering, ✂ first cut) remain. |
| E, F | Not started. |
...
- Wire `agentprobe_core.attacks` into `plan_attempts`/`run_suite`, so `attack`-only, `obfuscate` and `mutations` cases actually run instead of being refused up front. Needs `plan_attempts` (and its callers in the CLI, `runs.py`, `ci.py`, `worker.py`) to go async for the mutator's LLM call, or a separate sync expansion pass before planning for everything except `mutations`.
...
- C3: MCP adapter. C4: failure clustering (`GET /runs/{id}/findings`, populating `top_findings` in `/ci/report`'s response — currently always empty). ✂ C4 is the first cut if time runs short (POSITIONING.md).
