...
| B1.4 | HTTP adapter: request template, JSONPath response mapping, timeouts, retries/backoff, SSRF guard; trace step model; CLI-only Python adapter ([ADR 0012](decisions/0012-http-adapter-and-ssrf-guard.md)) | Unit tests, including private-IP blocking |
...
5. **Q5 — statistics method (#9).** Approved as proposed (the per-case correction was later amended to Tarone–Holm; see #9): Fisher exact per case with Holm correction, a paired sign-flip permutation test at suite level, a case-level bootstrap for the CI, α = 0.05, `min_drop` = 0.05. All of these are configurable via the suite YAML and CLI flags, not hardcoded, and the small-N power limitation is documented prominently (CLI `--help` and the docs). [ADR 0006](decisions/0006-statistics-methodology.md).
